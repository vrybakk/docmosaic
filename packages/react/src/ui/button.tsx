 ...
 * Editor Button.
 *
 * Mirrors the apps/web Button surface used by editor primitives, minus the
 * marketing-only `gradient` variant and the framer-motion/Typography deps.
 * Plain `<span>` is used in place of the app's `Typography` wrapper — text
 * styling is fully covered by the variant classes.
 */
                ...
                {typeof children === 'string' ? (
                    <span
                        className={cn(
                            ...
                            size === 'sm'
                                ? 'text-[0.75rem] leading-[100%]'
                                : 'text-[0.875rem] leading-[100%]',
                        )}
                    >
                        {children}
                    </span>
