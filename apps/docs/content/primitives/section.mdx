...
description: One section on the canvas — image, text, shape, or drawing. Handles drag, resize, image upload, and the hover/selected toolbar.
---
...
The default `Editor.Section` ships with image / text / shape / drawing renderers. If you need a custom variant, render your own component inside the canvas and skip `Editor.Section`:

```tsx
<Editor.Canvas>
    {sections.map((s) =>
        s.type === 'custom' ? <MyCustomSection key={s.id} section={s} /> : <Editor.Section key={s.id} />
    )}
</Editor.Canvas>
```
