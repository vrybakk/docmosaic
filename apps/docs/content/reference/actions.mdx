...
| Method                     | Effect                                                            |
| -------------------------- | ----------------------------------------------------------------- |
| `addSection(input)`        | Insert a new section. `input` carries `{ pageIndex, type, ... }`. |
| `updateSection(id, patch)` | Merge `patch` into the section identified by `id`.                |
| `deleteSection(id)`        | Remove the section. Clears selection if it was selected.          |
| `duplicateSection(id)`     | Clone the section onto the same page with a small offset.         |
