...
description: What changed in @docmosaic/react 2.0 - the editor-* token removal and the PageThumbnail reorder API - and how to update consumer code.
index: true
---
...
-   **One breaking primitive-prop change:** `Editor.PageThumbnail` now takes `onMovePage` instead of `dragHandlers` / `dropIndicators` (see [below](#editorpagethumbnail-reorder-api)). The PDF pipeline, document model, reducer, and history are otherwise unchanged.
...
| Removed          | Use instead                      |
| ---------------- | -------------------------------- |
| `dragHandlers`   | `onMovePage(fromIndex, toIndex)` |
| `dropIndicators` | `onMovePage(fromIndex, toIndex)` |
...
Open an issue on [GitHub](https://github.com/vrybakk/docmosaic/issues) if you hit anything not covered here.
