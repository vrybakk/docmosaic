...
<Callout type="warn">
    DocMosaic's image uploads land as **data URLs** (no server, no upload pipeline). `next/image`
    can't proxy data URLs through its optimizer, so the renderer passes `unoptimized` for them. Skip
    this and you'll get a runtime error on the first user upload.
</Callout>
