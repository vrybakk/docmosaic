...
<Callout type="info">
    Drop the add-page button for fixed-format documents (like single-page invoices) with `<Editor.Pages hideAddPage />`. The user can still reorder existing pages — just not add or remove them.
</Callout>
