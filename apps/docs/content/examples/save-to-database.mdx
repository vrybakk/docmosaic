    ...
    return (
        <div className="flex flex-col h-screen">
            <div className="border-b px-4 py-2 text-sm text-muted-foreground">
                {status === 'pending' ? 'Saving…' : status === 'saved' ? 'All changes saved' : status === 'error' ? 'Save failed — retrying' : ' '}
            </div>
...
<Callout type="warn">
    Image sections embed data URLs by default — a few photos can balloon a single document into MBs. For production, strip data URLs to object-store keys before saving and rehydrate them on load. See the [persisting templates recipe](/docs/recipes/persisting-templates) for the pattern.
</Callout>
