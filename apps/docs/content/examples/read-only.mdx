...
<Callout type="info">
    For a side-by-side "current vs reference" view, mix an editable canvas with `Editor.StaticCanvas` — same root, two surfaces, one frozen. See the [read-only recipe](/docs/recipes/read-only) for the pattern.
</Callout>
