...
## Example — converting a paper size for a tooltip

```ts
import { getPageDimensions, formatDimensions } from '@docmosaic/core';

formatDimensions(getPageDimensions('LETTER', 'mm'), 'mm');
// => '216×279mm'

formatDimensions(getPageDimensions('LETTER', 'in'), 'in');
// => '8.5×11in'
```
...
```ts
import { convertDimensions } from '@docmosaic/core';
...
// then mmToPt(width) / mmToPt(height) — or skip the intermediate via px * 72/96
const widthPt = img.naturalWidth * (72 / 96);
const heightPt = img.naturalHeight * (72 / 96);
```

The editor itself takes the shortcut: `px * 72/96`. Both paths are equivalent because 1mm = 96/25.4 px and 1mm = 2.83465pt.
...
- **`mmToPt`/`ptToMm` use the constant `2.83465`.** That's the rounded value of `72 / 25.4`. Errors are <1e-5 over A0 dimensions — well below jsPDF's own precision.
