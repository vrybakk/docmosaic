...
`Page.guides` is optional — legacy documents have no guides, and the renderer treats `undefined` as "no guides". The PDF generator never reads the field; the `generate.guides.test.ts` byte-diff gate locks that contract: rendering a document with guides set must produce exactly the same bytes as the same document with guides omitted.
...
Two new actions on the `@docmosaic/core` reducer write `Page.guides`. Both fail closed: out-of-range `pageIndex` returns the previous state untouched, and `ADD_GUIDE` skips exact duplicates so repeated drags onto the same value don't bloat the array.
...
- [Unit system](./unit-system.md) — why guides are stored in PDF points alongside section geometry
- [`packages/react/src/primitives/canvas/snap.ts`](../../packages/react/src/primitives/canvas/snap.ts) — the pure helpers `computeSnapTargets` / `computeSnap` / `computeGroupBBox`
- [`packages/core/src/types.ts`](../../packages/core/src/types.ts) — the `PageGuides` interface
- [`packages/core/src/pdf/generate.guides.test.ts`](../../packages/core/src/pdf/generate.guides.test.ts) — byte-diff gate proving guides never leak into the PDF
